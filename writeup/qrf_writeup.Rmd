...
pred_sf = dash2018_fr %>%
  ...
  mutate(Hab_Roll = if_else(SiteNam == 'Pahsimeroi_2018' & is.na(Hab_Roll),
                            0,
                            Hab_Roll)) %>%
  mutate_at(vars(SlowWater_Pct, WetSC_Pct),
            list(~ if_else(is.na(.), 0, .))) %>%
  mutate_at(vars(SlowWater_Pct, 
                 WetSC_Pct),
            list(~ .* 100)) %>%
...
bar_plot_list = pred_df %>%
  ...
  map(.f = function(x) {
    x %>%
      ...
      mutate_at(vars(Hab_Roll),
                list(as.factor)) %>%
      mutate(Hab_Roll = fct_reorder(Hab_Roll,
                                    chnk_per_m2)) %>%
      left_join(rel_imp_df %>%
              filter(Species == 'Chinook')) %>%
      mutate_at(vars(Metric, Name),
                list(~ fct_reorder(., desc(relImp)))) %>%
      ggplot(aes(x = Hab_Roll,
                 y = value,
                 fill = chnk_per_m2)) +
      geom_col() +
      scale_fill_viridis_c(direction = -1) +
