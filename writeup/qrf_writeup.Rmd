...
pred_sf = dash2018_hr_gaa %>%
  ...
  # slice(1) %>%
  # ungroup() %>%
  rename(avg_aug_temp = S2_02_11.y,
         LWVol_Wet = Wod_Vlm,
         # Q = MeanU_v1,
         Sin_CL = sinuosity,
...
```{r lem-map, fig.cap = "Geomorphic reaches in the Lemhi colored by average capacity (fish per m)."}
...
```{r pah-map, fig.cap = "Geomorphic reaches in the Pahsimeroi colored by average capacity (fish per m)."}
...
```{r ups-map, fig.cap = "Geomorphic reaches in the upper Salmon colored by average capacity (fish per m)."}
...
xy_plot_all = pred_df %>%
  ...
  mutate(Name = recode(Name,
                       'Substrate Est: Coarse and Fine Gravel' = 'Percent Gravel',
                       'Sinuosity Via Centerline' = 'Sinuosity',
                       'Wetted Side Channel Percent By Area' = 'Percent Side Channel',
                       'Percent Undercut by Length' = 'Percent Undercut',
                       'Wetted Width Integrated' = 'Wetted Width',
                       'Avg. August Temperature' = 'Avg. August Temp.',
                       'Wetted Channel Braidedness' = 'Braidedness',
                       'Large Wood Volume: Wetted' = 'LWD Frequency')) %>%
  ggplot(aes(x = value,
             y = chnk_per_m2)) +
  geom_smooth() +
...
box_plot_all = pred_df %>%
  ...
  facet_wrap(~ Name,
             scales = 'free_y',
             labeller = labeller(Name = as_labeller(box_plot_fnames))) +
