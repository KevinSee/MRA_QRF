...
gr_maps = gr_cap %>%
  map(.f = function(x) {
    flowlines = x %>%
      ...
      st_as_sfc() %>%
      st_buffer(dist = 1500) %>%
      QRFcapacity::get_flowlines(stream_order = 3,
                                 wtsd_polygon = .) %>%
      filter(!gnis_name %in% c("Lemhi River",
                               "Pahsimeroi River", 
                               "Bloody Dick Creek",
                               "Hidden Creek",
                               "Selway Creek",
    ...
    map_obj = x %>%
      ...
      pivot_longer(cols = matches('per_m'),
                   names_to = "type",
                   values_to = "avg_cap") %>%
      mutate(spp = if_else(grepl('^chnk', type),
                           "Chinook",
                           "Steelhead")) %>%
      group_by(spp, type) %>%
      mutate(rel_cap = (avg_cap - min(avg_cap, na.rm = T)) / (max(avg_cap, na.rm = T) - min(avg_cap, na.rm = T))) %>%
      ungroup() %>%
      filter(grepl('per_m$', type)) %>%
      st_as_sf() %>%
      ggplot() +
      geom_sf(data = flowlines,
          color = "gray60") +
      # geom_sf(aes(fill = avg_cap)) +
      # scale_fill_viridis_c(direction = -1,
      #                      name = expression('Capacity per'~m^2)) +
      geom_sf(aes(fill = rel_cap)) +
      scale_fill_viridis_c(direction = -1,
                           name = 'Relative Capacity Score') +
      geom_sf_label(aes(label = str_remove(GeoReach, "^GR-")),
                    size = 2,
                    nudge_x = 3000,
                    nudge_y = 500) +
    ...
    if(unique(x$Watershed) == "Lemhi") {
      flw_label = flowlines %>%
      arrange(gnis_name, Hydroseq) %>%
      select(gnis_name, Hydroseq, StreamOrde, TotDASqKM, LENGTHKM) %>%
      group_by(gnis_name) %>%
      summarize(Hydroseq = max(Hydroseq),
      ...
      map_obj = map_obj +
        geom_text(data = flw_label,
                aes(label = gnis_name,
                    geometry = geometry),
                color = "gray60",
                size = 2,
                stat = "sf_coordinates",
                nudge_x = if_else(grepl("Hayden", flw_label$gnis_name), -5000, 6000),
                nudge_y = if_else(grepl("Salmon", flw_label$gnis_name), 7000, 100))
        
    } else if(unique(x$Watershed) == "Pahsimeroi") {
      flw_label = flowlines %>%
        arrange(gnis_name, Hydroseq) %>%
        select(gnis_name, Hydroseq, StreamOrde, TotDASqKM, LENGTHKM) %>%
        group_by(gnis_name) %>%
      ...
      map_obj = map_obj +
        geom_text(data = flw_label,
                ...
                stat = "sf_coordinates",
                nudge_x = 3000,
                nudge_y = if_else(grepl("Salmon", flw_label$gnis_name), 1500, 
                                  if_else(grepl("Patterson", flw_label$gnis_name), 200, 100)))
    } else if(unique(x$Watershed == "Upper Salmon")) {
      flw_label = flowlines %>%
        arrange(gnis_name, Hydroseq) %>%
        select(gnis_name, Hydroseq, StreamOrde, TotDASqKM, LENGTHKM) %>%
        group_by(gnis_name) %>%
      ...
      map_obj = map_obj +
        coord_sf(xlim = st_bbox(x)[c(1,3)] + c(-1500, 1500)) +
        geom_text(data = flw_label,
            aes(label = gnis_name,
                geometry = geometry),
            color = "gray60",
            ...
            stat = "sf_coordinates",
            # nudge_x = 4000,
            nudge_x = if_else(grepl("Redfish", flw_label$gnis_name), 500, 
                              if_else(grepl("Alturas", flw_label$gnis_name), -2000, 2000)),
            nudge_y = if_else(grepl("Redfish", flw_label$gnis_name), -3000, 
                              if_else(grepl("Alturas", flw_label$gnis_name), 2000, 1000)))
    }
...
```{r lem-map, fig.cap = "Geomorphic reaches in the Lemhi colored by relative average capacity (fish per m^2)."}
gr_maps$Lemhi
```

```{r pah-map, fig.cap = "Geomorphic reaches in the Pahsimeroi colored by relative average capacity (fish per m^2)."}
gr_maps$Pahsimeroi
```

```{r ups-map, fig.cap = "Geomorphic reaches in the upper Salmon colored by relative average capacity (fish per m^2)."}
gr_maps$`Upper Salmon`
```
